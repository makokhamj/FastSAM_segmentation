...
# Add this import for the Agg backend
...
class FastSAMPrompt:
    ...
    def _get_bbox_from_mask(self, mask):
        ...
        if len(contours) > 1:
            for cnt in contours[1:]:
                ...
                x1, y1 = min(x1, x_t), min(y1, y_t)
                x2, y2 = max(x2, x_t + w_t), max(y2, y_t + h_t)
        return [x1, y1, x2, y2]
    ...
    def plot_to_result(self,
        ...
        if isinstance(annotations[0], dict):
            annotations = [ann['segmentation'] for ann in annotations]
        ...
        if better_quality:
            ...
            for mask in annotations:
                ...
                m = cv2.morphologyEx(m, cv2.MORPH_OPEN, np.ones((8,8), np.uint8))
                cleaned.append(m)
            annotations = cleaned

        # Render masks (CPU vs GPU)
        if self.device == 'cpu':
            self.fast_show_mask(np.array(annotations), ax,
                                random_color=mask_random_color,
                                bboxes=bboxes, points=points,
                                pointlabel=point_label,
                                retinamask=retina,
                                target_height=h, target_width=w)
        else:
            self.fast_show_mask_gpu(torch.from_numpy(np.array(annotations)), ax,
                                    random_color=mask_random_color,
                                    bboxes=bboxes, points=points,
                                    pointlabel=point_label,
                                    retinamask=retina,
        ...
        # Optionally overlay contours
        if withContours:
            all_contours = []
            for mask in annotations:
                ann = mask.astype(np.uint8)
                contours, _ = cv2.findContours(ann, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
                all_contours.extend(contours)
            contour_img = np.zeros((h, w, 1), dtype=np.uint8)
            cv2.drawContours(contour_img, all_contours, -1, (255,), 2)
            color = np.array([0, 0, 255, 204], dtype=np.uint8).reshape(1,1,4) / 255.0
            overlay = np.concatenate([contour_img, contour_img, contour_img, contour_img], axis=2) * color
            ax.imshow(overlay)

        # --- HERE: use an Agg canvas to get rgb buffer ---
            # ... after all plt drawing ...
    ...
    def plot(self,
        ...
        # ==== New start ====
        # 1) If it's a tensor, check emptiness and convert to numpy
        if isinstance(annotations, torch.Tensor):
            if annotations.numel() == 0:
                return None
            annotations = annotations.cpu().numpy()

        # 2) If it's a numpy array, check emptiness and convert to list
        elif isinstance(annotations, np.ndarray):
            if annotations.size == 0:
                return None
            annotations = list(annotations)

        # 3) Otherwise assume it's a Python sequence
        else:
            if not isinstance(annotations, (list, tuple)) or len(annotations) == 0:
                return None

        result = self.plot_to_result(
            annotations, bboxes, points, point_label,
            mask_random_color, better_quality,
            retina, withContours
        )
        os.makedirs(os.path.dirname(output_path), exist_ok=True)
        cv2.imwrite(output_path, result[:, :, ::-1])
        result = self.plot_to_result(annotations, bboxes, points, point_label,
                                     mask_random_color, better_quality,
                                     retina, withContours)
        os.makedirs(os.path.dirname(output_path), exist_ok=True)
